...
app = FastAPI(title="Global Context Synthesizer", version="1.0.0", lifespan=lifespan)


@app.get("/health")
async def health():
    latest = db.latest()
    return {"status": "OK", "service": "global-context-synthesizer", "version": "1.0.0", "latest_snapshot_id": (latest or {}).get("snapshot_id"), "latest_status": (latest or {}).get("status")}
