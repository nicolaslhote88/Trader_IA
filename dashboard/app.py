    ...
    with c2:
        with st.container(border=True):
            ...
            if th_df.empty:
                ...
                agg["dir"] = (
                    agg["dir"].astype(str).str.strip().str.lower().map({"bullish": "Bullish", "bearish": "Bearish"}).fillna("Neutral")
                )
                agg_plot = agg[agg["dir"].isin(["Bullish", "Bearish"])].copy()
                if agg_plot.empty:
                    agg_plot = agg.copy()
                agg_plot = agg_plot.sort_values("theme_score", ascending=False).head(8).sort_values("theme_score", ascending=True)
                show_legend = agg_plot["dir"].nunique() > 1
                fig_th = px.bar(
                    agg_plot,
                    x="theme_score",
                    y="theme",
