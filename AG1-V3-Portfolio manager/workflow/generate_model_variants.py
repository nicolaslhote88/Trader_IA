...
def patch_init_run_context_defaults(workflow: dict, cfg: dict) -> None:
    ...
    def replace_default(src: str, pattern: str, value: str, label: str) -> str:
        updated, count = re.subn(pattern, rf"\1{value}\2", src)
        if count < 1:
            raise ValueError(f"Init Run Context patch failed, pattern not found for {label}")
        return updated

    js_code = replace_default(
        js_code,
