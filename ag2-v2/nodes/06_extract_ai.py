...
for item in items:
    ...
    try:
        ...
        # Coherence guards
        if validated and decision != "APPROVE": validated = False
        if decision == "APPROVE" and stop_loss is None: decision = "WATCH"; validated = False
        if not reasoning: reasoning = "(no reasoning returned)"

        pass_pm = decision == "APPROVE" or (decision == "WATCH" and alignment == "WITH_BIAS" and quality >= 5)
        output_ref = "AG2AI|" + symbol + "|" + sig_hash

        ai_data = {"ai_decision": decision, "ai_validated": validated, "ai_quality": quality,
        ...
        con.execute("UPDATE technical_signals SET " + sets + ", updated_at = CURRENT_TIMESTAMP WHERE id = ?",
        ...
        out = dict(d)
        out["ai_decision"] = "ERROR"; out["ai_reasoning"] = str(e); out["should_vectorize"] = False
        results.append({"json": out})
