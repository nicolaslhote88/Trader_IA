...
with duckdb.connect(db_path) as con:
    ...
    initial_capital = float((brief.get("config") or {}).get("initial_capital_eur") or 10000.0)
    equity = initial_capital + floating + realized - fees
    pnl_total = equity - initial_capital
    row = con.execute("SELECT MAX(equity_eur) FROM core.portfolio_snapshot").fetchone()
    peak = max(float(row[0] or initial_capital), equity, initial_capital)
    dd_total = equity / peak - 1.0
    lev = notional / equity if equity > 0 else 0.0
    margin_used = notional / max(1.0, float((brief.get("config") or {}).get("leverage_max") or 2.0))
    ...
    con.execute(
        ...
        [
            ...
            floating, pnl_total,
            min(0.0, pnl_total / initial_capital), dd_total, "AG1-FX-V2 snapshot — 3 piliers",
        ],
    )
