...
CREATE TABLE IF NOT EXISTS core.model_proposals (
  proposal_id VARCHAR PRIMARY KEY,
  run_id VARCHAR NOT NULL REFERENCES core.runs(run_id),
  ts TIMESTAMPTZ NOT NULL,
  model_key VARCHAR NOT NULL,
  model_name VARCHAR,
  extractor_status VARCHAR,
  parse_ok BOOLEAN,
  decision_json JSON,
  actions_json JSON,
  warnings_json JSON,
...
CREATE TABLE IF NOT EXISTS core.consensus_decisions (
  ...
  status VARCHAR NOT NULL,
  reason VARCHAR,
  selected_qty DECIMAL(18,8),
  selected_weight_pct DOUBLE,
  selected_limit_price DOUBLE,
  confidence DOUBLE,
  order_id VARCHAR,
  payload_json JSON,
  created_at TIMESTAMPTZ DEFAULT now()
...
CREATE TABLE IF NOT EXISTS core.consensus_votes (
  vote_id VARCHAR PRIMARY KEY,
  run_id VARCHAR NOT NULL REFERENCES core.runs(run_id),
  consensus_id VARCHAR REFERENCES core.consensus_decisions(consensus_id),
  ts TIMESTAMPTZ NOT NULL,
  model_key VARCHAR NOT NULL,
  symbol VARCHAR NOT NULL,
  intent VARCHAR NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_model_proposals_run_model ON core.model_proposals(run_id, model_key);
CREATE INDEX IF NOT EXISTS idx_consensus_decisions_run_symbol ON core.consensus_decisions(run_id, symbol);
CREATE INDEX IF NOT EXISTS idx_consensus_votes_run_symbol ON core.consensus_votes(run_id, symbol);
