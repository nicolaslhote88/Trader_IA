...
def load_workflow():
    value = json.loads((ROOT / "AG4-SPE-IBKR-V1-workflow.json").read_text(encoding="utf-8-sig"))
...
class IbkrWorkflowContractTests(unittest.TestCase):
    ...
    def test_builder_is_idempotent(self):
        import build_ibkr_workflow

        workflow = load_workflow()
        rebuilt = build_ibkr_workflow.build(ROOT / "AG4-SPE-IBKR-V1-workflow.json")
