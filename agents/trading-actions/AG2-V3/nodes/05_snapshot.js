// AG2-V3 - Build AI Validation Context (actions/ETF/crypto only)
...
if (h1_entry !== null && (h1.action === "BUY" || h1.action === "SELL")) {
  if (h1.action === "BUY") {
    ...
    if (sl_atr !== null || sl_sup !== null) {
      stop_loss_suggested = Math.min(sl_atr !== null ? sl_atr : Infinity, sl_sup !== null ? sl_sup : Infinity);
  ...
  if (h1.action === "SELL") {
    ...
    if (sl_atr !== null || sl_res !== null) {
      stop_loss_suggested = Math.max(sl_atr !== null ? sl_atr : -Infinity, sl_res !== null ? sl_res : -Infinity);
...
let rr_meta = {};
if (h1_entry !== null && h1_res !== null && h1.action === "BUY" && stop_loss_suggested !== null) {
  const tpDist = h1_res - h1_entry;
  const slDist = h1_entry - stop_loss_suggested;
  rr_theoretical = tpDist > 0 && slDist > 0 ? Math.round((tpDist / slDist) * 100) / 100 : null;
  rr_meta = { method: "resistance_vs_stop", entry: h1_entry, tp: h1_res, sl: stop_loss_suggested, tp_dist: tpDist, sl_dist: slDist };
}

const bars = Array.isArray(item.h1_bars_60) ? item.h1_bars_60 : [];
const recentBars = bars.map((b) => ({ t: b.t, o: b.o, h: b.h, l: b.l, c: b.c, v: b.v }));
    ...
    json: {
      ...
      ai_context: {
        ...
        data_quality_flags: Array.isArray(item.data_quality_flags)
          ...
          : (() => {
              try { return JSON.parse(item.data_quality_flags || "[]"); } catch { return []; }
            })(),
