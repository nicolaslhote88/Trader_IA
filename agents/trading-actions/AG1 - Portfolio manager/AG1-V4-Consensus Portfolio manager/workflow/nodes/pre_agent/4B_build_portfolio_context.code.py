...
def load_portfolio_config(con, config_version="ag1_v4_consensus_v1"):
    if con is None:
        return {}
    try:
        row = con.execute(
        ...
        if row is None:
            return {}
        return {
            "initial_capital_eur": to_num(row[0], 10000.0) or 10000.0,
            "lot_close_method": row[1] or "FIFO",
            "default_fee_bps": to_num(row[2], 10.0) or 10.0,
            "kill_switch_active": bool(row[3]),
            "max_pos_pct": to_num(row[4], 25.0) or 25.0,
            "max_sector_pct": to_num(row[5], 40.0) or 40.0,
            "max_daily_drawdown_pct": to_num(row[6], 6.0) or 6.0,
            "portfolio_config_payload": row[7],
        }
    except Exception as exc:
        return {"portfolio_config_error": str(exc)[:256]}
...
with db_con(db_path) as con:
    if con is not None:
        portfolio_config = load_portfolio_config(con)
...
if portfolio_source == "duckdb_empty" and not positions and cash_eur <= 0 and total_value <= 0:
...
config_merged = {
    **(input0.get("config", {}) if isinstance(input0.get("config", {}), dict) else {}),
    **portfolio_config,
    "strategyVersion": (input0.get("run", {}) or {}).get("strategyVersion", "strategy_v4_consensus"),
    "configVersion": (input0.get("run", {}) or {}).get("configVersion", "ag1_v4_consensus_v1"),
    "promptVersion": (input0.get("run", {}) or {}).get("promptVersion", "prompt_v4_consensus"),
}
