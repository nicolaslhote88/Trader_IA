...
DEFAULT_DB_PATH = os.getenv("AG1_V4_DUCKDB_PATH", "/files/duckdb/ag1_v4_consensus.duckdb")
DEFAULT_TZ = "Europe/Paris"
DEFAULT_CONFIG_VERSION = "ag1_v4_consensus_v1"
SCHEMA_PATH = Path(__file__).resolve().parents[2] / "sql" / "portfolio_ledger_schema_v4.sql"
...
def _upsert_model_proposals(con: duckdb.DuckDBPyConnection, rows_in: Sequence[Mapping[str, Any]], run_id: str, default_ts: str) -> int:
    rows: List[Tuple[Any, ...]] = []
    for idx, r in enumerate(rows_in, start=1):
        model_key = _clean_text(r.get("model_key") or r.get("modelKey"), 64)
        if not model_key:
            continue
        rows.append(
            (
                _clean_text(r.get("proposal_id") or r.get("proposalId"), 128) or f"PROP|{run_id}|{model_key}|{idx:03d}",
                run_id,
                _parse_ts(r.get("ts"), default_ts),
                model_key,
                ...
                _json_text(r.get("actions_json") or r.get("actionsJson") or r.get("actions")),
                _json_text(r.get("warnings_json") or r.get("warningsJson") or r.get("warnings")),
                _clean_text(r.get("error"), 2048) or None,
                _to_int(r.get("latency_ms") or r.get("latencyMs"), None),
            )
        )
    if not rows:
        return 0
...
def _upsert_consensus_decisions(con: duckdb.DuckDBPyConnection, rows_in: Sequence[Mapping[str, Any]], run_id: str, default_ts: str) -> int:
    rows: List[Tuple[Any, ...]] = []
    for idx, r in enumerate(rows_in, start=1):
        symbol = _norm_symbol(r.get("symbol"))
        intent = _clean_text(r.get("intent"), 32).upper()
        if not symbol or not intent:
            continue
        rows.append(
            (
                _clean_text(r.get("consensus_id") or r.get("consensusId"), 128) or f"CONS|{run_id}|{symbol}|{intent}|{idx:03d}",
                run_id,
                _parse_ts(r.get("ts"), default_ts),
                symbol,
                intent,
                _clean_text(r.get("action"), 32).upper() or intent,
                _clean_text(r.get("side"), 16).upper() or None,
                _to_int(r.get("vote_count") or r.get("voteCount"), 0) or 0,
                _to_int(r.get("valid_model_count") or r.get("validModelCount"), 0) or 0,
                _clean_text(r.get("model_keys") or r.get("modelKeys"), 256) or None,
                _clean_text(r.get("status"), 64).upper() or "UNKNOWN",
                _clean_text(r.get("reason"), 2048) or None,
                _to_float(r.get("selected_qty") or r.get("selectedQty"), None),
                _to_float(r.get("selected_weight_pct") or r.get("selectedWeightPct"), None),
                _to_float(r.get("selected_limit_price") or r.get("selectedLimitPrice"), None),
                _to_float(r.get("confidence"), None),
                _clean_text(r.get("order_id") or r.get("orderId"), 128) or None,
                _json_text(r.get("payload_json") or r.get("payloadJson") or r),
            )
        )
    if not rows:
        return 0
    con.executemany(
        """
        INSERT INTO core.consensus_decisions (
          consensus_id, run_id, ts, symbol, intent, action, side, vote_count, valid_model_count,
          model_keys, status, reason, selected_qty, selected_weight_pct, selected_limit_price,
          confidence, order_id, payload_json
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT (consensus_id) DO UPDATE SET
          ...
          valid_model_count = excluded.valid_model_count,
          model_keys = excluded.model_keys,
          status = excluded.status,
          reason = excluded.reason,
          selected_qty = excluded.selected_qty,
          selected_weight_pct = excluded.selected_weight_pct,
          selected_limit_price = excluded.selected_limit_price,
          confidence = excluded.confidence,
          order_id = excluded.order_id,
          payload_json = excluded.payload_json
        """,
...
def _upsert_consensus_votes(con: duckdb.DuckDBPyConnection, rows_in: Sequence[Mapping[str, Any]], run_id: str, default_ts: str) -> int:
    rows: List[Tuple[Any, ...]] = []
    for idx, r in enumerate(rows_in, start=1):
        model_key = _clean_text(r.get("model_key") or r.get("modelKey"), 64)
        symbol = _norm_symbol(r.get("symbol"))
        intent = _clean_text(r.get("intent"), 32).upper()
        if not model_key or not symbol or not intent:
            continue
        rows.append(
            (
                _clean_text(r.get("vote_id") or r.get("voteId"), 128) or f"VOTE|{run_id}|{model_key}|{symbol}|{intent}|{idx:03d}",
                run_id,
                _clean_text(r.get("consensus_id") or r.get("consensusId"), 128) or None,
                _parse_ts(r.get("ts"), default_ts),
                model_key,
                symbol,
                intent,
                _clean_text(r.get("action"), 32).upper() or None,
                _clean_text(r.get("side"), 16).upper() or None,
                _to_bool(r.get("executable"), default=False),
                _to_float(r.get("confidence"), None),
                _to_float(r.get("target_qty") or r.get("targetQty"), None),
                _to_float(r.get("target_weight_pct") or r.get("targetWeightPct"), None),
                _to_float(r.get("limit_price") or r.get("limitPrice"), None),
                _clean_text(r.get("rationale"), 2048) or None,
                _json_text(r.get("payload_json") or r.get("payloadJson") or r),
            )
        )
    ...
    con.executemany(
        """
        INSERT INTO core.consensus_votes (
          vote_id, run_id, consensus_id, ts, model_key, symbol, intent, action, side, executable,
          confidence, target_qty, target_weight_pct, limit_price, rationale, payload_json
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
def init_schema(db_path: str = DEFAULT_DB_PATH) -> Dict[str, Any]:
    sql_path = Path(os.getenv("AG1_V4_LEDGER_SCHEMA_PATH") or os.getenv("AG1_LEDGER_SCHEMA_PATH", str(SCHEMA_PATH)))
    ...
    with _db_con(db_path) as con:
        ...
        con.execute(
            ...
            [
                ...
                _iso_now(),
                _json_text({
                    "seeded_by": "duckdb_writer.init_schema",
                    "system": "ag1_v4_consensus",
                    "starting_cash_eur": 10000.0,
                }),
            ],
        )
        con.execute(
            ...
            VALUES (?, NULL, ?, ?, ?, ?, NULL, ?, ?, ?)
            ON CONFLICT (cash_tx_id) DO NOTHING
            """,
            [
                "INIT_CASH_AG1_V4_CONSENSUS",
                _iso_now(),
                "EUR",
                10000.00,
                "DEPOSIT",
                "ag1_v4_consensus_initial_capital",
                "Initial AG1 V4 consensus portfolio cash",
                _json_text({
                    "seeded_by": "duckdb_writer.init_schema",
                    "system": "ag1_v4_consensus",
                    "starting_cash_eur": 10000.0,
...
def upsert_run_bundle(db_path: str, bundle_json: Any) -> Dict[str, Any]:
    ...
    with _db_con(db_path) as con:
        ...
        try:
            ...
            rows["model_proposals"] = _upsert_model_proposals(con, bundle.get("model_proposals") or [], run_id, default_ts)
            rows["consensus_decisions"] = _upsert_consensus_decisions(con, bundle.get("consensus_decisions") or [], run_id, default_ts)
            rows["consensus_votes"] = _upsert_consensus_votes(con, bundle.get("consensus_votes") or [], run_id, default_ts)
